...
<?php
$sub_menu = array(
     array("url" => "/role/user_search/", "name" => "社員別権限検索", "action" => "user_search", "action2" => "user_detail")
    ,array("url" => "/role/organization_search/", "name" => "部署別権限検索", "action" => "organization_search", "action2" => "organization_detail")
    ,array("url" => "/role/subscribes/", "name" => "DRAGONマスタ連携処理", "action" => "subscribes", "action2" => "")
);
foreach ($sub_menu as $val) {
    if ($this->Acl->check($this->Auth->user("role_id"), $val["url"])) {
        if ( ($val["action"] == $this->action) || ($val["action2"] == $this->action) ) {
