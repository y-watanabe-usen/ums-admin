...
(function($) {
    $(function() {
        ...
        //未発送ダウンロードボタン
        $("#bt_no_publish").click(function() {
            $.blockUI({message: '発送データを作成しています。<BR>時間がかかる場合があります。'});
            $("#dl_message").text("");
            $('#fr_dl input[name="type"]').val("no_publish");
            $("#fr_dl").submit();
//            $.unblockUI();
        });
    });
...
<div class="main-title" style="left:210px;">発送データダウンロード</div>
...
<table style="padding:0;margin:0;position:absolute;top:0; width: 95%;">
    <tr>
        <td><button id="bt_no_publish">発送データを作成する</button></td>
        <td style="text-align: right;">
        <?php
        if (!empty($unis_batch_result["end_datetime"])) {
            echo "UNIS最終連携日時：{$unis_batch_result["end_datetime"]}";
        }
        ?>
        </td>
     </tr>
...
<form id="fr_dl" action="/issue/publish_download" method="POST">
